...
# parser.add_argument('--Conv_PE', type=int, default=130, help='the number of PE allocated for Conv')
...
# parser.add_argument('--hybrid',  type=str, default='cifar100_all1', help='the type of hybrid models')
...
# bit_conv=args.bit_conv
# bit_shift=args.bit_shift
# bit_Adder=args.bit_Adder
# Ratio_Adder = 1.88
# Ratio_Shift = 1.88
...
def valid_tiling(tiling_factor, type, num_PE, memory, dataflow, FC_flag):
    valid = True
    ratio = 1
    if type == 'shift' or type == 'add':
        ratio = 6/8
    # ####################### RS ##############################
    if dataflow == 'rs':
        M3 = tiling_factor["M3"]
    ...
    # ####################### WS ##############################
        ...
        if not (C1*M1*E1*F1 < num_PE):
            valid = False
        # ########## feature map ############
        if not (C0*R*S < 48):
            valid = False
        # ############# weight #############
        if not (C0*M0 < 112/ratio):
            valid = False
        # ############# GB ##############
...
def get_latency(tiling_factor, unit_latency, type, dataflow):
    # ####################### RS ##############################
    if type == 'shift' or type == 'add':
        number = 8
    elif type == 'conv':
    ...
    if dataflow == 'rs': 
        ...
        latency = M3*M2*M0*C3*C2*E3*F*unit_latency*math.ceil(C0*S/number)
    ...
    # ####################### IS ##############################
        ...
        latency = M0*C3*C2*E3*E0*F3*F0*unit_latency*math.ceil(C0*R*S/number)
    ...
    # # ####################### WS ##############################
        ...
        stride = tiling_factor["stride"]
        F = tiling_factor["F"]
        latency = M3*M2*M0*C3*C2*E0*F0*unit_latency*math.ceil(C0*R*S/number)
        ...
        stride = tiling_factor["stride"]
        F = tiling_factor["F"]
        latency = M3*M2*M0*E3*E0*F3*F0*unit_latency*math.ceil(C0*R*S/number)
    
    return latency

def get_energy(tiling_factor, unit_energy, type, dataflow='rs'):
    # ####################### RS ##############################
    if type == 'shift' or type == 'add':
        number = 8
    elif type == 'conv':
    ...
    if dataflow == 'rs':
        ...
        RF_ofmap = ( max(C3 * C2 * C1 * R * (math.ceil(C0 * S/number) - 1 ) * 2, 1) ) * num_ofmap
    ...
    # ####################### IS ##############################
        ...
        RF_ofmap = ( max(C3 * C2 * C1 * (math.ceil(C0*R*S/number) - 1 ) * 2, 1) ) * num_ofmap
    ...
    # ####################### OS ##############################
        ...
        RF_ifmap = M3 * M2 * M1 * M0 * num_ifmap * R * E / H * S * F / W 
        RF_weight = E3 * F3 * E1* F1 * E0 * F0 * num_weight
        RF_ofmap = ( max(C1 * (math.ceil(C0*R*S/number) - 1 ) * 2, 1) ) * num_ofmap
    ...
    energy = computation * unit_energy["unit_Conv"] \
            + (DRAM_ifmap + DRAM_weight/ratio + DRAM_ofmap) * unit_energy["unit_DRAM"] \
            + (DRAM_ifmap + DRAM_ofmap) * unit_energy["unit_DRAM_GB"] \
            + (GB_ifmap + GB_ofmap) * unit_energy["unit_GB"] \
            + (NoC_ifmap + NoC_weight/ratio + NoC_ofmap) * unit_energy["unit_NoC"] \
            + (RF_ifmap + RF_weight/ratio + RF_ofmap) * unit_energy["unit_RF"]
    return energy, computation * unit_energy["unit_Conv"], (DRAM_ifmap + DRAM_weight/ratio + DRAM_ofmap) * unit_energy["unit_DRAM"], (GB_ifmap + GB_ofmap) * unit_energy["unit_GB"], (NoC_ifmap + NoC_weight/ratio + NoC_ofmap) * unit_energy["unit_NoC"], (RF_ifmap + RF_weight/ratio + RF_ofmap) * unit_energy["unit_RF"]
...
def get_OPs_HW_metric(layer_dict, PE, type, memory, dataflow, v=False):
    # constant defination
    bit = 8
    ...
    unit_energy["unit_DRAM"] = 200/(1e9) * 0.63 * bit/16 # mJ/16 bits
    unit_energy["unit_DRAM_GB"] = 0.0/(1e9) * bit/16  # mJ/16 bits
    unit_energy["unit_GB"] = 6.0/(1e9) * 0.63 * bit/16  # mJ/16 bits
    unit_energy["unit_NoC"] = 2.0/(1e9) * 0.63 * bit/16  # mJ/16 bits
    # unit_energy["unit_NoC_psum"] = 1.0/(1e9) * 1   # mJ/16 bits
    unit_energy["unit_RF"] = 1.0/(1e9) * 0.63 * bit/16  # mJ/16 bits    
    ...
    # if bit == 32:
    #     unit_latency = 1.0/(250e6)*(1e3) * 4  # ms
    # elif bit == 16:
    #     unit_latency = 1.0/(250e6)*(1e3)  # ms
    # elif bit == 8:
    #     unit_latency = 1.0/(250e6)*(1e3) / 2  # ms
    unit_latency = 1.0/(250e6)*(1e3)  # ms
        ...
        for M_tile in M_tile_list:
            for C_tile in C_tile_list:
                for E_tile in E_tile_list:
                    ...
                    tiling_factor["E1"] = E_tile[0]
                    tiling_factor["E3"] = E_tile[1]
                    if valid_tiling(tiling_factor, type, PE, memory, dataflow, FC_flag):
                        
                        latency = get_latency(tiling_factor, unit_latency, type, dataflow)
                        # if (latency < latency_constraint):
                        energy, compute, DRAM, GB, NoC, RF = get_energy(tiling_factor, unit_energy, type, dataflow)
                        energy_list.append(energy)
                        compute_energy_list.append(compute)
                        DRAM_energy_list.append(DRAM)
                        GB_energy_list.append(GB)
...
# ################################# PE Allocation ########################################
def predictor(OPs_list, stage, Conv_PE): 
    flops_conv = 0
    flops_shift = 0
    flops_Adder = 0
    para_conv = 0
    ...
    # TODO:
    computation_cost = round(((flops_conv*0.213 + flops_shift*0.07
 + flops_Adder*0.05)/1e6),3)
    # FIXME:
    if (flops_shift + flops_Adder) == 0:
        ratio = 1
    else:
        ratio = flops_conv/(flops_shift + flops_Adder)
    ...
    # print(flops_conv,flops_shift,flops_Adder)
    ...
    # Conv_PE = args.Conv_PE
    # TODO:
    Switchable_PE = (168-Conv_PE)//1.22
    # Shift_PE = Switchable_PE*flops_shift//(flops_shift+flops_Adder)
    # Adder_PE = Switchable_PE-Shift_PE
    Shift_PE = 0
    Adder_PE = 0

    # ############## customized ################
    # flops_shift = flops_shift/8
    # flops_Adder = flops_Adder/8
    # Ratio_Shift = 1/1.14
    # Ratio_Adder = 1/1.03
    # per_flops = args.total_PE/(flops_conv + flops_shift/Ratio_Shift + flops_Adder/Ratio_Adder)
    # print(flops_conv,flops_shift,flops_Adder)
    ...
    # print("-------------------------------> The PE allocation is (conv, shift, adder): ",(Conv_PE, Shift_PE, Adder_PE))
    # num_type = 0
    # if Conv_PE!=0:
    #     num_type += 1
    # if Shift_PE!=0:
        ...
        for s in range(1):
            for a in range(1): 
                ...
                for i in range(100000):
                    ...
                    if i == 0 :
                        if shift != None:
                            shift_energy, shift_latency, shift_min_energy, shift_min_latency, shift_compute_energy, shift_DRAM_energy, shift_GB_energy, shift_NoC_energy, shift_RF_energy, shift_memory = get_OPs_HW_metric(shift, PE=Shift_PE, type='shift', memory=0, dataflow=dataflow[s], v=False)
                        ...
                        if Adder != None:
                            Adder_energy, Adder_latency, Adder_min_energy, Adder_min_latency, Adder_compute_energy, Adder_DRAM_energy, Adder_GB_energy, Adder_NoC_energy, Adder_RF_energy, Adder_memory = get_OPs_HW_metric(Adder, PE=Adder_PE, type='add', memory=shift_memory, dataflow=dataflow[a], v=False)
                        ...
                        if conv != None:
                            conv_energy, conv_latency, conv_min_energy, conv_min_latency, conv_compute_energy, conv_DRAM_energy, conv_GB_energy, conv_NoC_energy, conv_RF_energy, conv_memory = get_OPs_HW_metric(conv, PE=Conv_PE, type='conv', memory=shift_memory+Adder_memory, dataflow=dataflow[c], v=False)
                            if conv_energy != None:
                    ...
                    if (Conv_total_latency == min(Shift_total_latency, Conv_total_latency, Adder_total_latency)):
                        ...
                        if shift != 'over':
                            ...
                            if Shift_total_latency == Adder_total_latency:
                                Adder_memory = 0
                            shift_energy, shift_latency, shift_min_energy, shift_min_latency, shift_compute_energy, shift_DRAM_energy, shift_GB_energy, shift_NoC_energy, shift_RF_energy, shift_memory = get_OPs_HW_metric(shift, PE=Shift_PE, type='shift', memory=conv_memory+Adder_memory, dataflow=dataflow[s], v=False)
                            if shift_energy != None:
                                Shift_total_energy += shift_energy
                                Shift_total_latency += shift_latency
                                Shift_total_min_energy += shift_min_energy
                        ...
                        if Adder != 'over':
                            ...
                            if Adder_total_latency == Shift_total_latency:
                                shift_memory = 0
                            Adder_energy, Adder_latency, Adder_min_energy, Adder_min_latency, Adder_compute_energy, Adder_DRAM_energy, Adder_GB_energy, Adder_NoC_energy, Adder_RF_energy, Adder_memory = get_OPs_HW_metric(Adder, PE=Adder_PE, type='add', memory=conv_memory+Adder_memory, dataflow=dataflow[a], v=False)
                            if Adder_energy != None:
                                Adder_total_energy += Adder_energy
                                Adder_total_latency += Adder_latency
                                Adder_total_min_energy += Adder_min_energy
    ...
    # print(' ')
    # print("!!!!!!!!!!!!!!!!!!!!! Best Latency !!!!!!!!!!!!!!!!!!!!!")
    # print("------------------------------------> The best of all: energy: {}, energy_woDRAM: {} (compute: {}, DRAM: {}, GB: {}, NoC: {}, RF: {}) mJ, latency: {} ms, EDP:{}".format(
    # total_energy[latency_optimal_tiling_factor_idx[0]],
    # total_wo_DRAM_energy[latency_optimal_tiling_factor_idx[0]],
    # total_compute_energy[latency_optimal_tiling_factor_idx[0]],
    # total_DRAM_energy[latency_optimal_tiling_factor_idx[0]],
    # total_GB_energy[latency_optimal_tiling_factor_idx[0]],
    # total_NoC_energy[latency_optimal_tiling_factor_idx[0]],
    # total_RF_energy[latency_optimal_tiling_factor_idx[0]],
    # total_latency[latency_optimal_tiling_factor_idx[0]],
    # total_latency[latency_optimal_tiling_factor_idx[0]]*total_energy[latency_optimal_tiling_factor_idx[0]]
    # ))
    # print("------------------------------------> The chosed dataflows for Conv/ Shift/ Adder are: ",(dataflow[latency_optimal_tiling_factor_idx[0]//shift_dataflow//Adder_dataflow], 
    # dataflow[latency_optimal_tiling_factor_idx[0]//Adder_dataflow % shift_dataflow], 
    # dataflow[latency_optimal_tiling_factor_idx[0] % Adder_dataflow]
    # ))
    ...
    total_latency[total_optimal_tiling_factor_idx[0]]*total_energy[total_optimal_tiling_factor_idx[0]]))
    ...
    EDP = total_latency[total_optimal_tiling_factor_idx[0]]*total_energy[total_optimal_tiling_factor_idx[0]]
    return round(EDP,3), computation_cost