...
# from .kernels
# from .ste as ste
from .utils import get_shift_and_sign
from .kernels import linear, conv2d
from .ste import clampabs, round_power_of_2, round_fixed_point
# import .ste as ste
...
class LinearShiftQFunction(Function):
    ...
    def forward(ctx, input, weight, bias=None, conc_weight=None, use_kernel=False, use_cuda=True, rounding='deterministic', shift_range=(0,-15)):
        ...
        shift, sign = get_shift_and_sign(weight, rounding)
        ...
        if use_kernel:
            ...
            out = linear(input_fixed_point, shift, sign, bias_fixed_point, conc_weight, use_cuda)
            out = out.float()
...
class LinearShiftQ(nn.Module):
    ...
    def forward(self, input):
        self.weight.data = clampabs(self.weight.data, 2**self.shift_range[0], 2**self.shift_range[1])
        weight_q = round_power_of_2(self.weight, self.rounding)
        input_fixed_point = round_fixed_point(input)
        if self.bias is not None:
            bias_fixed_point = round_fixed_point(self.bias)
        else:
...
class Conv2dShiftQFunction(Function):
    ...
    def forward(ctx, input, weight, bias=None, conc_weight=None, stride=1, padding=0, dilation=1, groups=1, use_kernel=False, use_cuda=False):
        ...
        shift, sign = get_shift_and_sign(weight, rounding='deterministic')
        ...
        if use_kernel:
            ...
            out = conv2d(input_fixed_point, shift, sign, bias_fixed_point, conc_weight, stride, padding, dilation, groups, use_cuda)
...
class _ConvNdShiftQ(nn.Module):
    ...
    def reset_parameters(self):
        # TODO:
        # init.kaiming_uniform_(self.weight, a=math.sqrt(5))
        # init.kaiming_uniform_(self.weight, mode='fan_out')
...
class Conv2dShiftQ(_ConvNdShiftQ):
    ...
    #@weak_script_method
    def forward(self, input, weight=None, padding=None, group=None, bias=False):
        if self.shift_or_full == 'shift':
            if weight != None:
                weights = clampabs(weight.data, 2**self.shift_range[0], 2**self.shift_range[1])
            else:
                weights = clampabs(self.weight.data, 2**self.shift_range[0], 2**self.shift_range[1])
            weight_q = round_power_of_2(weights, self.rounding)
            # print(weight_q)
            input_fixed_point = round_fixed_point(input, quant_bits=self.quant_bits)
            if bias is not False:
                bias_fixed_point = round_fixed_point(bias,quant_bits=self.quant_bits)
            elif self.bias is not None:
                bias_fixed_point = round_fixed_point(self.bias,quant_bits=self.quant_bits)
            else:
                bias_fixed_point = None
        ...
        if self.use_kernel:
            ...
            if group == None:
                group = self.groups
            return torch.nn.functional.conv2d(input_padded, weight_q, bias_fixed_point, self.stride, padding, self.dilation, group)
