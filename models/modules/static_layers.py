...
class SEShiftLayer(nn.Module):
    REDUCTION = 4

    def __init__(self, channel):
        super(SEShiftLayer, self).__init__()
        ...
        self.fc = nn.Sequential(OrderedDict([
                            # ('reduce', nn.Conv2d(self.channel, num_mid, 1, 1, 0, bias=True)),
                            ('reduce', Shiftlayer(self.channel, num_mid, 1, stride=1, bias=True, padding=0,)),
                            ('relu', nn.ReLU(inplace=True)),
                            # ('expand', nn.Conv2d(num_mid, self.channel, 1, 1, 0, bias=True)),
                            ('expand', Shiftlayer(num_mid, self.channel, 1, stride=1, bias=True, padding=0,)),
                            ('h_sigmoid', Hsigmoid(inplace=True)),
        ]))
...
class SEAdderLayer(nn.Module):
    REDUCTION = 4

    def __init__(self, channel):
        super(SEAdderLayer, self).__init__()
        ...
        self.fc = nn.Sequential(OrderedDict([
                            # ('reduce', nn.Conv2d(self.channel, num_mid, 1, 1, 0, bias=True)),
                            ('reduce', Adderlayer(self.channel, num_mid, 1, stride=1, bias=True, padding=0,)),
                            ('relu', nn.ReLU(inplace=True)),
                            # ('expand', nn.Conv2d(num_mid, self.channel, 1, 1, 0, bias=True)),
                            ('expand', Adderlayer(num_mid, self.channel, 1, stride=1, bias=True, padding=0,)),
                            ('h_sigmoid', Hsigmoid(inplace=True)),
        ]))
        ...
        self.stage = 100
        # self.reduce_affine = nn.Parameter(torch.randn(self.stage))
        # self.expand_affine = nn.Parameter(torch.randn(self.stage))
        # TODO: 
...
class ShortcutShiftLayer(MyModule):

    def __init__(self, in_channels, out_channels, reduction=1):
        super(ShortcutShiftLayer, self).__init__()
        ...
        self.conv = Shiftlayer(
            in_channels, out_channels, 1, stride=1, bias=False, padding=0,
        )
    ...
    def forward(self, x):
        if self.reduction > 1:
            ...
            x = F.avg_pool2d(x, self.reduction, padding=padding)
        if self.in_channels != self.out_channels:
            x = self.conv(x, weight=None, padding=0, group=1)
        return x
    ...
    @staticmethod
    def build_from_config(config):
        return ShortcutShiftLayer(**config)   


class ShortcutAdderLayer(MyModule):

    def __init__(self, in_channels, out_channels, reduction=1):
        super(ShortcutAdderLayer, self).__init__()
    ...
    def forward(self, x):
        if self.reduction > 1:
            ...
            x = F.avg_pool2d(x, self.reduction, padding=padding)
        if self.in_channels != self.out_channels:
            x = self.conv(x, weight=False, padding=0, group=1)
        return x
    ...
    @staticmethod
    def build_from_config(config):
        return ShortcutAdderLayer(**config)   
...
class MBInvertedShiftLayer(MyModule):

    def __init__(self, in_channels, out_channels,
                 kernel_size=3, stride=1, expand_ratio=6, mid_channels=None, act_func='relu6', use_se=False, channels_per_group=1):
        super(MBInvertedShiftLayer, self).__init__()
        ...
        if self.expand_ratio == 1:
            ...
            self.inverted_bottleneck = nn.Sequential(OrderedDict([
                # ('conv', nn.Conv2d(self.in_channels, feature_dim, 1, 1, 0, bias=False)),
                ('shift', Shiftlayer(self.in_channels, feature_dim, 1, 1, 0, bias=False)),
                ('bn', nn.BatchNorm2d(feature_dim)),
                ('act', build_activation(self.act_func, inplace=True)),
            ]))

        assert feature_dim % self.channels_per_group == 0
        self.active_groups = feature_dim // self.channels_per_group
        self.pad = get_same_padding(self.kernel_size)
        depth_conv_modules = [
            # ('conv', nn.Conv2d(feature_dim, feature_dim, kernel_size, stride, pad, groups=active_groups, bias=False)),
            ('shift', Shiftlayer(feature_dim, feature_dim, kernel_size, stride=stride, padding=self.pad, groups=self.active_groups)),
            ('bn', nn.BatchNorm2d(feature_dim)),
            ('act', build_activation(self.act_func, inplace=True))
        ]
        if self.use_se:
            depth_conv_modules.append(('se_shift', SEShiftLayer(feature_dim)))
        self.depth_conv = nn.Sequential(OrderedDict(depth_conv_modules))

        self.point_linear = nn.Sequential(OrderedDict([
            # ('conv', nn.Conv2d(feature_dim, out_channels, 1, 1, 0, bias=False)),
            ('shift', Shiftlayer(feature_dim, out_channels, 1, 1, 0, bias=False)),
            ('bn', nn.BatchNorm2d(out_channels)),
        ]))

    def forward(self, x):
        if self.inverted_bottleneck:
            self.inverted_bottleneck[0].padding = (0,0)
            self.inverted_bottleneck[0].groups = 1
            x = self.inverted_bottleneck(x)
        
        self.depth_conv[0].padding = _pair(self.pad)
        # print('padding',_pair(self.pad))
        self.depth_conv[0].groups = self.active_groups
        x = self.depth_conv(x)
        
        self.point_linear[0].padding = (0,0)
        self.point_linear[0].groups = 1
        x = self.point_linear(x)
        return x
    ...
    @staticmethod
    def build_from_config(config):
        return MBInvertedShiftLayer(**config)


class MBInvertedAdderLayer(MyModule):

    def __init__(self, in_channels, out_channels,
                 kernel_size=3, stride=1, expand_ratio=6, mid_channels=None, act_func='relu6', use_se=False, channels_per_group=1):
        super(MBInvertedAdderLayer, self).__init__()
        ...
        if self.expand_ratio == 1:
            ...
            self.inverted_bottleneck = nn.Sequential(OrderedDict([
                # ('conv', nn.Conv2d(self.in_channels, feature_dim, 1, 1, 0, bias=False)),
                ('adder', Adderlayer(self.in_channels, feature_dim, 1, 1, 0, bias=False)),
                ('bn', nn.BatchNorm2d(feature_dim)),
                ('act', build_activation(self.act_func, inplace=True)),
            ]))
        ...
        depth_conv_modules = [
            # ('conv', nn.Conv2d(feature_dim, feature_dim, kernel_size, stride, pad, groups=active_groups, bias=False)),
            ('adder', Adderlayer(feature_dim, feature_dim, kernel_size, stride=stride, padding=self.pad, groups=self.active_groups)),
            ('bn', nn.BatchNorm2d(feature_dim)),
            ('act', build_activation(self.act_func, inplace=True))
        ]
        if self.use_se:
            depth_conv_modules.append(('se_adder', SEAdderLayer(feature_dim)))
        self.depth_conv = nn.Sequential(OrderedDict(depth_conv_modules))

        self.point_linear = nn.Sequential(OrderedDict([
            # ('conv', nn.Conv2d(feature_dim, out_channels, 1, 1, 0, bias=False)),
            ('adder', Adderlayer(feature_dim, out_channels, 1, 1, 0, bias=False)),
            ('bn', nn.BatchNorm2d(out_channels)),
        ]))

    def forward(self, x):
        if self.inverted_bottleneck:
            self.inverted_bottleneck[0].padding = 0
            self.inverted_bottleneck[0].groups = 1
            x = self.inverted_bottleneck(x)
        
        self.depth_conv[0].padding = self.pad
        # print('padding',_pair(self.pad))
        self.depth_conv[0].groups = self.active_groups
        x = self.depth_conv(x)
        
        self.point_linear[0].padding = 0
        self.point_linear[0].groups = 1
        x = self.point_linear(x)
        return x
    ...
    @staticmethod
    def build_from_config(config):
        return MBInvertedAdderLayer(**config)
...
class MobileInvertedResidualBlock(MyModule):
    ...
    def forward(self, x):
        ...
        if self.mobile_inverted_conv is None: # or isinstance(self.mobile_inverted_conv, ZeroLayer):
            ...
            # print('self.mobile_inverted_conv',self.mobile_inverted_conv.type)
